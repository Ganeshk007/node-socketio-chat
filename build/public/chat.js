...
var users = document.querySelector('.users'); //Socket server URL

var socket = io.connect('http://localhost:3000'); //Fetch URL Params from URL

var queryString = window.location.search;
var urlParams = new URLSearchParams(queryString);
var username = urlParams.get('username');
var roomname = urlParams.get('roomname');
console.log(username, roomname); //Display the roomname the user is connected to

roomMessage.innerHTML = "Connected in room ".concat(roomname); //Emitting username and roomname of newly joined user to server
...
socket.on('online-users', function (data) {
  ...
  data.forEach(function (user) {
    users.innerHTML += "<p>".concat(user, "</p>");
  });
});